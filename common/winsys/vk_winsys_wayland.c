...
vkin_winsys_create_surface (vk_t *vk)
{
    ...
    s_window.window_size.width  = s_win_w;
    s_window.window_size.height = s_win_h;
    s_window.display = &s_display;
    toggle_fullscreen (&s_window, 0);

    s_window.wlEGLNativeWindow =  wl_egl_window_create(s_window.wlSurface, s_win_w, s_win_h);
    ...
    surfaceCreateInfo.display = s_display.wlDisplay;
    surfaceCreateInfo.surface = s_window.wlSurface;
    VK_CHECK (vkCreateWaylandSurfaceKHR (vk->instance, &surfaceCreateInfo, NULL, &surface));
